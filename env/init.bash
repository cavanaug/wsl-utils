...
declare -x WIN_WINDIR='/mnt/c/WINDOWS'
...
if [[ ! -d "/mnt/wsl/${WSL_DISTRO_NAME}" ]]; then
    ...
    ${WIN_WINDIR}/System32/wsl.exe -d "${WSL_DISTRO_NAME}" -u root mount --bind / "/mnt/wsl/${WSL_DISTRO_NAME}/"
fi
...
if [[ ! -f "${WIN_COMSPEC}" ]]; then
    WIN_ERROR="ERROR: Windows system32 cmd.exe not found at ${CMD}"
    echo "${WIN_ERROR}"
    return
fi
